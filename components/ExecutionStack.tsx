...
export const ExecutionStack: React.FC<ExecutionStackProps> = ({ 
  ...
  return (
    <div className="w-full bg-[#0C0C0E] border border-white/10 p-1 flex flex-col h-full shadow-2xl">
      <div className="bg-[#141417] p-3 md:p-4 flex justify-between items-center border-b border-white/5">
        <div className="flex items-center space-x-2 md:space-x-3">
            <div className={`w-2 h-2 ${isProcessing ? 'bg-accent animate-pulse' : 'bg-white/20'}`} />
            <h3 className="font-mono text-[10px] md:text-[11px] font-bold tracking-widest text-white/80 uppercase">Execution_Stack_V0</h3>
        </div>
        <span className="font-mono text-[8px] md:text-[9px] text-white/40 uppercase">Cluster: AX-9</span>
      </div>

      <div className="flex-grow p-3 md:p-4 space-y-2 overflow-y-auto bg-[#08080A]">
          ...
          <div key={layer.id} className={`group transition-all duration-200 border-l-2 ${
            ...
            <div className="p-2 md:p-3 flex items-center justify-between">
              <div className="flex items-center space-x-3 md:space-x-4">
                <span className="font-mono text-[9px] md:text-[10px] text-white/20 w-4">0{idx + 1}</span>
                <span className={`text-[11px] md:text-xs font-bold tracking-tight uppercase ${
              ...
              <div className="flex items-center space-x-3">
                <span className={`font-mono text-[8px] md:text-[9px] uppercase tracking-tighter ${
                  layer.status === 'pass' ? 'text-accent' : 
                  layer.status === 'fail' ? 'text-red-500' : 
            ...
            {(layer.status !== 'idle' && layer.status !== 'suppressed') && (
              <div className="px-8 md:px-11 pb-2 md:pb-3 font-mono">
                {layer.logs.map((log, lIdx) => (
                  <div key={lIdx} className="text-[8px] md:text-[9px] text-white/30 lowercase leading-tight py-0.5 border-t border-white/[0.03]">
      ...
      <div className="p-3 md:p-4 bg-[#141417] border-t border-white/5 flex justify-between items-center">
        <div className="text-[8px] md:text-[9px] font-mono text-white/30">
            SYSTEM_AWAITING_INTENT
        </div>
        {!isProcessing && (
          <button 
            onClick={() => { resetStack(); setTimeout(runSequence, 100); }}
            className="text-accent underline underline-offset-4 font-bold text-[9px] md:text-[10px] tracking-widest uppercase hover:text-white transition-all active:text-white"
          >
            Run Stack
          </button>
        )}
